...
<h1 className="text-3xl text-center mt-3 mb-3">Easy Steps</h1>
        ...
        <div className={`banner flex items-center justify-between m-auto w-11/12 text-gray-700 `}>
          <div className={`absolute left-0 right-0 h-10 rounded-2xl  mx-auto w-11/12 ${step1?'bg-green-50':(step2? 'bg-green-100':'bg-green-100')}`}>
          </div>

          <div className={`step relative w-32 h-32 flex rounded-full flex-col justify-center items-center z-3 text-center rounded  ${step1 ? ' bg-green-50 w-44' : ''}`} id="step1">
            <img src="/images/check.png" alt="" width={'30px'} className="z-10" />
            <p className={`${step1 ? 'animate- visible' : 'hidden'}`}>Select</p>
          </div>
          <div className={`step relative w-32 h-32 flex rounded-full flex-col justify-center items-center z-3 text-center rounded ${step2 ? 'bg-green-100 w-44' : ''}`} id="step1">
            <img src="/images/cart.png" alt="" width={'30px'} className="z-10" />
            <p className={`${step2 ? 'animate- visible' : 'hidden'}`}>Add to cart</p>
          </div>
          <div className={`step relative w-32 h-32 flex rounded flex-col justify-center items-center z-3 text-center rounded-full ${step3 ? 'animate-pulse	 bg-green-200 w-44 ' : ''}`} id="step1">
